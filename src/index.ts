...
class Vditor extends VditorMethod {
    ...
    constructor(id: string | HTMLElement, options?: IOptions) {
        ...
        if (!mergedOptions.i18n) {
            if (!["de_DE", "en_US", "es_ES", "fr_FR", "ja_JP", "ko_KR", "pt_BR", "ru_RU", "sv_SE", "vi_VN", "zh_CN", "zh_TW"].includes(mergedOptions.lang)) {
                ...
                addScript(`${mergedOptions.cdn}/i18n/${mergedOptions.lang}.js`, i18nScriptID).then(() => {
                    this.init(id as HTMLElement, mergedOptions);
                }).catch(error => {
                    this.showErrorTip(`GET ${mergedOptions.cdn}/i18n/${mergedOptions.lang}.js net::ERR_ABORTED 404 (Not Found)`);
                });
            }
        } else {
    ...
    private init(id: HTMLElement, mergedOptions: IOptions) {
        ...
        addScript(
            ...
            `${mergedOptions.cdn}/lute/lute.min.js`,
            "vditorLuteScript",
        ).then(() => {
            ...
            if (mergedOptions.icon) {
                // 防止初始化 2 个编辑器时加载 2 次
                addScriptSync(`${mergedOptions.cdn}/icons/${mergedOptions.icon}.js`, "vditorIconScript");
            }
        });
    }
}
